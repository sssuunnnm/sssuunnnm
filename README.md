<div align="center">

# sssuunnnm

Full-stack Developer · Kotlin·Compose, React, Spring Boot로 서비스를 만듭니다.

`Kotlin` `TypeScript` `Java` `Python` `React` `Spring Boot` `FastAPI` `Docker` `AWS`

</div>

## Education

| 과정 | 기관 | 기간 |
| --- | --- | --- |
| 삼성청년SW·AI아카데미(SSAFY) 14기 | 삼성전자 | 2025.07 ~ 2026.06 |
| 캠퍼스 SW 아카데미 TABA 4기 | 티맥스티베로·단국대학교 | 2023.09 ~ 2023.12 |

## Projects

| 프로젝트 | 역할 | 스택 | 주관 | 성과 |
| --- | --- | --- | --- | --- |
| NGRAS | FE·BE·디자인 | React 19, FastAPI, Spring Boot, Kubernetes | 삼성전자·SSAFY | 프로젝트 1위 · 전시발표회 1위 (전국 113팀) |
| ShadowEng | PL·FE·인프라 | Kotlin, Compose, Docker, Jenkins, AWS | 삼성전자·SSAFY | 1위 |
| OneStep | UI/UX·FE | Kotlin, Compose, Mapbox, WebSocket | 삼성전자·SSAFY | 3위 |
| 라온트립 | FE 리더 | Flutter, Dart | 한국관광공사 | 장려상 |
| 실시간 멀티플레이 미니게임 | 1인 기획·개발·운영 | Firebase | 개인 | 실사용자 대상 운영 |

## Certifications

| 자격증 | 취득일 |
| --- | --- |
| AWS SAA-C03 | 2026.07 |
| ADsP | 2025.09 |
| SQLD | 2024.04 |
| 한국사능력검정 2급 | 2025.02 |

## Algorithm

<img src="http://mazassumnida.wtf/api/generate_badge?boj=sssuunnnm" alt="BOJ Tier" width="260" />

## Contact

<a href="https://github.com/sssuunnnm"><img alt="GitHub" src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white"/></a>
<a href="https://sssuunnnm.github.io/dev-archive/"><img alt="Blog" src="https://img.shields.io/badge/Dev--Archive-4A7FC1?style=flat-square&logo=astro&logoColor=white"/></a>
